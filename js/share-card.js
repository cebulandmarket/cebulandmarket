/**
 * CebuLandMarket - Share Card Generator
 * Creates 1080x1080 downloadable JPG images for property listings.
 */
...
function generateShareCard(listing) {
  ...
  var OWNER_LISTINGS = ['1', '2'];
  function applyFee(price, id) {
    if (id && OWNER_LISTINGS.indexOf(id) !== -1) return price;
  ...
  var price = formatPrice(applyFee(listing.total_price, listing.id));
  var location = getLocationName(listing.location) + ', Cebu';
  var area = listing.lot_area ? listing.lot_area.toLocaleString() + ' sqm' : '';
  var title = listing.title || 'Property for Sale';
  ...
  img.onload = function() {
    ...
    downloadCanvas(canvas, 'CebuLandMarket-' + (listing.id || 'property') + '.jpg');
  };
...
function drawBottom(ctx, title, price, location, area) {
  ...
  ctx.textAlign = 'left';
  ctx.fillText('CebuLandMarket', 48, brandY + 46);

  // URL
  ctx.fillStyle = 'rgba(255,255,255,0.8)';
  ctx.font = '500 24px -apple-system, BlinkMacSystemFont, sans-serif';
  ctx.textAlign = 'right';
  ctx.fillText('cebulandmarket.com', 1032, brandY + 46);
}
